    ...
    for (const file of files) {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001/api"}/products/upload-image`,
          {
            method: "POST",
            headers: {
    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        try {
          ...
          showError(
            t("admin.products.modals.create.error_uploading_images"),
            t("admin.products.modals.create.error_uploading_images_message"),
          );
      ...
      onProductCreated();
      showSuccess(
        t("admin.products.modals.create.product_created"),
        t("admin.products.modals.create.product_created_with_images"),
      );
    } catch (err: unknown) {
      // Extraer mensaje de error del API client
      let errorMessage = "";
      if (
        err &&
        typeof err === "object" &&
      ...
      if (errorMessage.toLowerCase().includes("sku")) {
        ...
        showError(
          t("admin.products.modals.create.error_creating"),
          errorMessage ||
            t("admin.products.modals.create.error_creating_message"),
        );
      }
    } finally {
      setUploadingImages(false);
      ...
      <div className="relative top-20 mx-auto p-5 border w-auto shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
        <div className="mt-3">
          <h3 className="text-lg font-medium text-center text-gray-900 mb-4">
            {t("admin.products.modals.create.title")}
          </h3>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                  {t("admin.products.modals.create.product_name")}
                </label>
                <input
                  type="text"
                  placeholder={t(
                    "admin.products.modals.create.product_name_placeholder",
                  )}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                  {t("admin.products.modals.create.description")}
                </label>
                <input
                  type="text"
                  placeholder={t(
                    "admin.products.modals.create.description_placeholder",
                  )}
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <span className="mt-2 text-sm text-black font-medium text-center leading-tight">
                      {t("admin.products.modals.create.main_image")}
                    </span>
                  </div>
                  {/* Otras imágenes */}
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">
                    {t("admin.products.modals.create.stock")}
                  </label>
                  <input
                    type="number"
                    min="0"
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">
                    {t("admin.products.modals.create.units_per_box")}
                  </label>
                  <input
                    type="number"
                    min="0"
                    ...
                    onChange={(e) =>
                      setCreateForm({
                        ...createForm,
                        units_per_box: parseInt(e.target.value) || 0,
                      })
                    }
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-black"
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">
                    {t("admin.products.modals.create.list_price")}
                  </label>
                  <input
                    type="number"
                    placeholder={t(
                      "admin.products.modals.create.list_price_placeholder",
                    )}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">
                    {t("admin.products.modals.create.wholesale_price")}
                  </label>
                  <input
                    type="number"
                    placeholder={t(
                      "admin.products.modals.create.wholesale_price_placeholder",
                    )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">
                    {t("admin.products.modals.create.iva")}
                  </label>
                  <input
                    type="number"
                    placeholder={t(
                      "admin.products.modals.create.iva_placeholder",
                    )}
                    min="0"
                    max="100"
                    step="0.01"
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                  {t("admin.products.modals.create.category")}
                </label>
                <select
                ...
                >
                  <option value="">
                    {loadingCategories
                      ? t("admin.products.modals.create.loading_categories")
                      : t("admin.products.modals.create.select_category")}
                  </option>
                  ...
                  {(() => {
                    ...
                    const childrenByParent = {} as Record<
                      string,
                      typeof categories
                    >;
            ...
            <div className="col-span-1 md:col-span-2 flex justify-end gap-4 mt-8">
              ...
              <Button
                ...
                text={
                  uploadingImages
                    ? t("admin.products.modals.create.uploading")
                    : t("admin.products.modals.create.upload_product")
                }
