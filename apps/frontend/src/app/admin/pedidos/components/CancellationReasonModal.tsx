...
interface CancellationReasonModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (reason: string, cancellationDate: string) => void;
  orderId: string;
  isLoading?: boolean;
}
...
export const CancellationReasonModal: React.FC<CancellationReasonModalProps> = ({
  ...
  React.useEffect(() => {
    if (isOpen && !cancellationDate) {
      const today = new Date();
      const todayStr = today.toISOString().slice(0, 16); // Format: YYYY-MM-DDTHH:MM
      setCancellationDate(todayStr);
    }
  }, [isOpen, cancellationDate]);

  const handleConfirm = () => {
    if (reason.trim() && cancellationDate) {
      onConfirm(reason.trim(), cancellationDate);
    }
  };
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {t('admin.orders.cancellation_modal.date_label')}
            </label>
            <input
              type="datetime-local"
              value={cancellationDate}
              onChange={(e) => setCancellationDate(e.target.value)}
              className="block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
              disabled={isLoading}
            />
            <Text size="xs" color="gray-500" className="mt-1">
              {t('admin.orders.cancellation_modal.date_help')}
            </Text>
          </div>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button
              ...
              disabled={!reason.trim() || !cancellationDate || isLoading}
            ...
            >
              {isLoading ? t('admin.orders.cancellation_modal.updating') : t('admin.orders.cancellation_modal.confirm')}
            </button>
          </div>
        </div>
      </div>
