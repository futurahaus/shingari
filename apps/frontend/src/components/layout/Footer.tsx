      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex flex-col items-center">
        ...
        <nav className="w-full mb-8">
          <ul className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-24">
            <li className="font-normal text-[14px] leading-6 tracking-[0px] text-center" >
              <Link href="/" className="hover:text-blue-600 transition-colors">
                <Text as='span' size="sm" color='gray-900'>{t('footer.home')}</Text>
              </Link>
            </li>
            <li className="font-normal text-[14px] leading-6 tracking-[0px] text-center" ><Link href="/contacto" className="hover:text-blue-600 transition-colors"><Text as='span' size="sm" color='gray-900'>{t('footer.contact')}</Text></Link></li>
            <li className="font-normal text-[14px] leading-6 tracking-[0px] text-center" ><Link href="/brands" className="hover:text-blue-600 transition-colors"><Text as='span' size="sm" color='gray-900'>{t('footer.brands')}</Text></Link></li>
            <li className="font-normal text-[14px] leading-6 tracking-[0px] text-center" >
              <Link href="/privacy-policy" className="hover:text-blue-600 transition-colors">
                <Text as='span' size="sm" color='gray-900'>{t('footer.privacy_policy')}</Text>
              </Link>
            </li>
            <li className="font-normal text-[14px] leading-6 tracking-[0px] text-center" >
              <Link href="/cookie-policy" className="hover:text-blue-600 transition-colors">
                <Text as='span' size="sm" color='gray-900'>{t('footer.cookie_policy')}</Text>
              </Link>
            </li>
          </ul>
        </nav>
